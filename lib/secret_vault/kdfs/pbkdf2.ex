  ...
  cond do
    Code.ensure_loaded?(:crypto) && function_exported?(:crypto, :pbkdf2_hmac, 5) ->
      ...
      defp do_kdf(user_input, salt, iterations_count, key_length) do
        :crypto.pbkdf2_hmac(:sha512, user_input, salt, iterations_count, key_length)
      end

    Code.ensure_loaded?(Pbkdf2KeyDerivation) && function_exported?(Pbkdf2KeyDerivation, :pbkdf2!, 5) ->
      defp do_kdf(user_input, salt, iterations_count, key_length) do
        Pbkdf2KeyDerivation.pbkdf2!(user_input, salt, :sha512, iterations_count, key_length)
      end

    true ->
      raise CompileError, description: "It seems your OTP version is under 24.2 and you didn't " <>
